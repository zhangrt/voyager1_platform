package system

import (
	"time"
)

type SysAuthority struct {
	CreatedAt       time.Time       // 创建时间
	UpdatedAt       time.Time       // 更新时间
	DeletedAt       *time.Time      `sql:"index"`
	AuthorityId     string          `json:"authorityId" gorm:"not null;unique;primary_key;comment:角色ID;size:90"` // 角色ID
	AuthorityName   string          `json:"authorityName" gorm:"comment:角色名"`                                    // 角色名
	ParentId        string          `json:"parentId" gorm:"comment:父角色ID"`                                       // 父角色ID
	DataAuthorityId []*SysAuthority `json:"dataAuthorityId,omitempty" gorm:"many2many:sys_data_authority_id;"`
	Children        []SysAuthority  `json:"children,omitempty" gorm:"-"`
	SysBaseMenus    []SysBaseMenu   `json:"menus,omitempty" gorm:"many2many:sys_authority_menus;"`
	Users           []SysUser       `json:"-" gorm:"many2many:sys_user_authority;"`
	DefaultRouter   string          `json:"defaultRouter" gorm:"comment:默认菜单;default:dashboard"` // 默认菜单(默认dashboard)
}

func (SysAuthority) TableName() string {
	return "sys_authorities"
}

func (SysAuthority) Init() SysAuthority {
	authority := SysAuthority{
		AuthorityId:     "",
		AuthorityName:   "",
		ParentId:        "",
		DataAuthorityId: []*SysAuthority{},
		Children:        []SysAuthority{},
		SysBaseMenus:    []SysBaseMenu{},
		Users:           []SysUser{},
		DefaultRouter:   "",
	}

	return authority
}
