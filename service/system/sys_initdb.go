...
/*
该初始化可在未进行数据库配置时，通过数据初始化接口对数据库进行初始化
*/
...
func (initDBService *InitDBService) InitDB(conf request.InitDB) (err error) {
	...
	case "cockroach":
		initHandler = NewCockroachInitHandler()
		ctx = context.WithValue(ctx, "dbtype", "cockroach")
	default:
		initHandler = NewMysqlInitHandler()
		ctx = context.WithValue(ctx, "dbtype", "mysql")
	}
...
// InitData 数据库数据初始化
func (initDBService *InitDBService) InitData(conf request.InitDB) (err error) {
	ctx := context.TODO()
	if len(initializers) == 0 {
		return errors.New("无可用初始化过程，请检查初始化是否已执行完成")
	}
	...
	ctx = context.WithValue(ctx, "db", global.GS_DB)
